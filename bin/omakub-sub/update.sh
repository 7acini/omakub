...
CHOICES=(
	"Omakdeb        Update Omakdeb itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"omakdeb") INSTALLER_FILE="$OMAKDEB_PATH/bin/omakdeb-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$OMAKDEB_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$OMAKDEB_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
